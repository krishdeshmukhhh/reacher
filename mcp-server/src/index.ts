...
interface Automation {
  ...
  filters: { minGmvScore: number; emailRequired: boolean; niches: string[] };
  status: string;
  createdAt: string;
}
...
function search_creators(args: {
  ...
  let creators = readJSON<Creator[]>('creators.json');

  if (args.niche) creators = creators.filter((c) => c.niche === args.niche);
  if (args.region) creators = creators.filter((c) => c.region === args.region);
  if (args.minFollowers !== undefined) creators = creators.filter((c) => c.followers >= args.minFollowers!);
  if (args.maxFollowers !== undefined) creators = creators.filter((c) => c.followers <= args.maxFollowers!);
  if (args.minGmvScore !== undefined) creators = creators.filter((c) => c.gmvScore >= args.minGmvScore!);
...
function create_creator_list(args: { listName: string; creatorIds: string[] }): CreatorList {
  const lists = readJSON<CreatorList[]>('lists.json');
...
function get_creator_list(args: { listName: string }): { list: CreatorList; creators: Creator[] } | { error: string } {
  const lists = readJSON<CreatorList[]>('lists.json');
  const list = lists.find((l) => l.name.toLowerCase() === args.listName.toLowerCase());
  if (!list) return { error: `No list found named "${args.listName}"` };
  const allCreators = readJSON<Creator[]>('creators.json');
...
function draft_automation(args: {
  ...
  const automations = readJSON<Automation[]>('automations.json');
...
function get_list_analytics(args: { listName: string }): object {
  const analytics = readJSON<{ lists: Record<string, object> }>('analytics.json');
  const lists = readJSON<CreatorList[]>('lists.json');
  const list = lists.find((l) => l.name.toLowerCase() === args.listName.toLowerCase());
  if (!list) return { error: `No list found named "${args.listName}"` };
...
function summarize_creator(args: { creatorId: string }): object {
  const creators = readJSON<Creator[]>('creators.json');
  const creator = creators.find((c) => c.id === args.creatorId);
  if (!creator) return { error: `Creator ${args.creatorId} not found` };
  ...
  if (creator.gmvScore >= 85) fitSignals.push('top-tier GMV performance');
  if (creator.sampleAcceptanceRate >= 0.8) fitSignals.push('high sample acceptance rate');
  if (creator.engagementRate >= 9) fitSignals.push('exceptional engagement rate');
  if (creator.emailAvailable) fitSignals.push('email available for direct outreach');
...
const server = new Server(
  { name: 'creator-ops-mcp', version: '1.0.0' },
  { capabilities: { tools: {} } }
);
  ...
  try {
    ...
    switch (name) {
      case 'search_creators':
        result = search_creators(args as Parameters<typeof search_creators>[0]);
        break;
      case 'create_creator_list':
        result = create_creator_list(args as Parameters<typeof create_creator_list>[0]);
        break;
      case 'get_creator_list':
        result = get_creator_list(args as Parameters<typeof get_creator_list>[0]);
        break;
      case 'draft_automation':
        result = draft_automation(args as Parameters<typeof draft_automation>[0]);
        break;
      case 'get_list_analytics':
        result = get_list_analytics(args as Parameters<typeof get_list_analytics>[0]);
        break;
      case 'summarize_creator':
        result = summarize_creator(args as Parameters<typeof summarize_creator>[0]);
        break;
      default:
